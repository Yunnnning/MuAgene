...
| `internal/artifacts/p2_plan/preprocessing_plan.json` | S0 ingest (`plan_assembler.assemble_plan` + `write_plan`) | `plan-review` renderer and all stages (the default layer; parameters.yaml overrides it) | Frozen plan snapshot; rebuilt only when S0 is rerun. |
