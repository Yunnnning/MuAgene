...
| `internal/artifacts/<stage>/*` | the stage `run()` | downstream stages; reports | Stage outputs (h5ad/parquet/bed/...). Some QC intermediates are deleted at the post_qc gate unless `retain_for_integration`. The S3 post-doublet h5ads (`s3_doublets/{rna,atac}_post_doublet.h5ad`) are **transient**: `qc_handoff` deletes them once the canonical post-QC h5mu exists, which S4/S5 read (S3's durable declared output is `calls.parquet`). |
