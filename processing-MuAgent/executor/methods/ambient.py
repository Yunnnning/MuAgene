...
required at runtime. If R / Bioconductor is unavailable or the requested
package is missing, the wrappers raise `AmbientUnavailable`; the S1a stage
catches that and records `s1a_ambient.method = "skipped_no_r"` instead of
failing the pipeline, so users without R still get a working pipeline.
"""
...
class AmbientUnavailable(RuntimeError):
    """R / DecontX / SoupX is not installed on this host."""
...
def _run_rscript(script: Path, args: list[str], *, log_file: Path | None = None) -> None:
    ...
    if rscript is None:
        raise AmbientUnavailable("Rscript is not on PATH; install R + Bioconductor.")
...
def run_decontx(
    ...
    *,
    ...
    if not all(pkgs.values()):
        ...
        raise AmbientUnavailable(
            f"DecontX requires R packages {missing}; install via "
            "BiocManager::install(c('celda','jsonlite'))."
        )
...
def run_soupx(
    ...
    *,
    ...
    if not all(pkgs.values()):
        missing = [p for p, ok in pkgs.items() if not ok]
        raise AmbientUnavailable(
            f"SoupX requires R packages {missing}; install via "
            "BiocManager::install(c('SoupX','jsonlite'))."
        )
