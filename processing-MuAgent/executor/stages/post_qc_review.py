"""post_qc_review — QC checkpoint between S3 and dimensionality reduction.

Generates user-facing QC figures (doublet score histograms, cell-count waterfall)
and writes an early QC summary markdown at
  deliverables/post_run/summary/qc_summary_pre_dimred.md

The stage marks itself awaiting approval so the user can review QC results
before S4 (RNA normalization) and S5 (ATAC LSI) are allowed to run.

Note: RNA QC violin figures (pre/post filter) and ATAC fragment size distribution
are already generated by S1 and S2 respectively; they appear in
deliverables/post_run/figures/ alongside the new figures written here.
"""
...
def _plot_cell_count_waterfall(run_dir: Path, figs_dir: Path) -> list[Path]:
    ...
    # Drop stages where both counts are missing
...
def propose(run_dir: Path | str) -> dict[str, Any]:
    ...
    summary_path = rp.qc_summary_pre_dimred_md
    summary_path.parent.mkdir(parents=True, exist_ok=True)
    try:
        summary_path.write_text(_qcs.build(run_dir))
    except Exception as e:
        log_event(run_dir, {"stage": "post_qc_review", "event": "summary_failed",
                             "error": str(e)})
        summary_path.write_text(f"# QC Summary (pre-dimred)\n\n_Error building summary: {e}_\n")
