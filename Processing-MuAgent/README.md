...
- **post_qc_review** — **QC review checkpoint (#2).** Propose-only gate between S3 and S6 PCA (RNA) + neighbor graph. Generates doublet histograms (with the chosen doublet threshold drawn as a red cutoff line), a cell-count waterfall (with counts labelled on bars), and `deliverables/qc/qc_review_<run>.md` — a plain-language summary of what each filter step did (MAD outlier bounds, MT/ribo ceilings, TSS enrichment, nucleosome signal, FRiP, union doublet policy). Each RNA/ATAC section opens with cells before filtering, retained, and removed. Revise quality-filter thresholds and re-run affected stages before approving. On approval, the large QC-only working files are automatically deleted to free storage: the QC matrices `rna_qc.h5ad`, `atac_qc.h5ad`, `atac_snap.h5ad`, the `qc_explore/atac_snap_explore.h5ad` import, the **S0 raw RNA ingest `s0_ingest/rna_ingest.h5ad` (~200 MB) + the unused `metadata_minimal.tsv`** (`rna_ingest.h5ad` is a pure cache — S1a reconstructs it via `io.load_rna_ingest` if a later re-process needs it, so its deletion never forces an S0 re-run), the **S1a ambient-corrected `s1a_ambient/rna_decontaminated.h5ad` (~400 MB)**, and the S1a recompute caches (`tsne_coords_cache.parquet`, `cell_totals.parquet`). The chr-normalised fragment caches `atac_fragments_cbf[_chrnorm].tsv.gz` (the single biggest artifact, ~1 GB) are **retained by default** — Integration-MuAgent re-counts them against a consensus peak set — and are deleted here only when `run.yaml` sets `retain_for_integration: false`. None of the deleted files is a declared Snakemake output: every QC/ingest stage declares only its durable marker (`s0_ingest/validation_report.json`, `s1a_ambient/summary.json`, `s1`/`s2 qc_summary.json`) and the downstream rule edges depend on those markers (S1a ← `validation_report.json`, S1 ← `summary.json`), so deletion never makes an output "missing" or triggers a re-run; none is read by a post-gate stage (S4+ read the post-QC h5mu). Preserved: those durable markers, the QC-metrics parquets (the final S8 manifest reads `qc_metrics_post.parquet`), and all S3+ artifacts. Because the S0/S1a RNA caches are removed here, the before/after-ambient marker-gene check (which reads `rna_decontaminated.h5ad`) is necessarily a **pre-approval** step. (Per-stage scratch dirs — `_work_soupx`/`_work_decontx`, `macs3_tmp` — are removed by their own stage as soon as it finishes, not here.)
...
- **S5 ATAC spectral embedding and peak matrix export** — SnapATAC2 tile matrix (`bin_size=500`, unified with S3) → feature selection → spectral embedding. In parallel, exports a feature (cell-by-feature) matrix using this priority order for the peak coordinates:
  ...
  3. **S2 / post-QC peaks** — BED at `deliverables/qc/peaks_<run>.bed` (moved by `qc_handoff` from S2 MACS3/ARC peaks); reused here with no redundant peak calling (`s2_peaks_macs3` / `s2_peaks_arc` / `user_peaks` mode).
...
- **S6 PCA (RNA) + neighbor graph** (`s6_neighbors`) — **RNA:** optional `sc.pp.scale`, then PCA; `n_pcs` from a chord-distance elbow on explained variance, capped at `rna_n_pcs_max`; nearest-neighbors on PCA space. **ATAC:** KNN graph on the S5 spectral embedding (`X_spectral` via `snap.pp.knn`). Working artifact `internal/artifacts/s6_neighbors/rna_neighbors.h5ad` (read by S7 by path; removed by `finish-cleanup`); the durable stage marker is `neighbors_summary.json`.
...
- **finish-cleanup** — Run by the agent at completion (`executor finish-cleanup`), **after** validating the S8 processed deliverable exists. Deletes the large S4–S8 intermediate working files (~0.7 GB/run) — content-duplicates of the final outputs: `s4_rna_norm/rna_norm.h5ad`, `s5_atac_spectral/{atac_spectral.h5ad, feature_matrix.npz, feature_names.tsv, feature_kind.txt, peak_matrix_*.h5ad, *_prepared.bed}`, `s6_neighbors/rna_neighbors.h5ad`, `s7_clustering/{rna_clustered.h5ad, atac_leiden_labels.parquet}`. **Refuses if the processed h5mu/h5ad is missing or empty**, so a failed run keeps its intermediates and can resume mid-pipeline. None is a declared Snakemake output; the durable per-stage markers (`norm_summary.json`, `spectral_summary.json`, `neighbors_summary.json`, `clustering_summary.json`, `s8_done.txt`) and all deliverables are preserved, so `status` still reports S4–S8 done and `submit --target all` does not re-run them. (The QC gate's `_cleanup_qc_intermediates` handles the QC-phase working files separately at `post_qc_review` approval; the ~1.1 GB fragment cache is retained there for Integration-MuAgent and is **not** touched by `finish-cleanup`.)
- **qc_handoff** — After QC approval (gated on `post_qc_review.approved`; its declared Snakemake dep is the durable S3 marker `calls.parquet`, and it reads the S3 post-doublet h5ads by path). Writes `deliverables/qc/post_qc_<run>.h5mu` (post-QC, post-doublet, **un-normalized** cells; h5mu on all branches — paired, separate, rna_only, atac_only), `deliverables/qc/post_qc_manifest.json` (schema `muagene.post_qc_handoff/1`), and on ATAC-bearing branches **`deliverables/qc/peaks_<run>.bed`** (per-sample peak coordinates moved from S2 internal artifacts or copied from `atac_peaks_path`), then **deletes the now-redundant internal `s3_doublets/{rna,atac}_post_doublet.h5ad`**. The **h5mu is the canonical post-QC store** — S4/S5 read it (so qc_handoff is upstream of the finish batch); `calls.parquet`, `joint_barcodes.txt`, `overlap_summary.json`, and prepared fragments are kept under `internal/`. `rule all` requires both this bundle and `run_manifest.json`. Independently buildable via `run --target qc_handoff`.
...
<run_dir>/
  deliverables/
    ...
    qc/                           ← QC checkpoint (#2): reports + post-QC Integration handoff
      ...
      peaks_<run>.bed              ← after approval (ATAC; per-sample peak coordinates)
    ...
    results/                      ← final deliverables (S8 + manifest)
      ...
      layout.json
  internal/
    artifacts/sN_<stage>/         ← intermediate stage outputs. Each stage keeps a small durable
                                     done-marker (qc_summary.json, norm_summary.json, spectral_summary.json,
                                     neighbors_summary.json, clustering_summary.json, s8_done.txt) that
                                     carries status + the Snakemake DAG edge; the large S4–S8 working
                                     h5ads/sidecars are removed by `finish-cleanup` once S8's processed
                                     deliverable exists (markers survive, so stages stay "done").
