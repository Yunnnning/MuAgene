...
> 4. **Unpaired RNA + ATAC** — RNA + ATAC from independent samples; no integration, each flows through its own pipeline.
>
...
| "unpaired", "RNA + ATAC from different samples" | `unpaired` |

Confirm in one sentence, and list the checkpoints relevant to their branch:

**For `paired` and `unpaired`:**

> Got it — you're running the **[paired / unpaired]** branch at `<run_dir>`.
>
...
>
> After QC approval I create and verify the post-QC handoff, then ask before starting
> the unattended normalization, clustering, and UMAP finish batch.
>
...
> Two inputs — looks like **paired multiome**. If the modalities are actually unpaired samples (no shared barcodes expected), say so and I'll switch to the `unpaired` branch. Otherwise I'll treat them as paired.
...
> I don't guess run locations. Where should I put the outputs? A fresh empty directory
> anywhere on your filesystem works. It will contain internal pipeline state and
> user-facing plan, QC, figure, and result deliverables.
