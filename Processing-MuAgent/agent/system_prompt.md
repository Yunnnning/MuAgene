...
2. **Step 2 — Collect paths, biological context, and execution mode (local vs HPC) + device (cpu/gpu).** See [`stage_prompts/inputs_intake.md`](stage_prompts/inputs_intake.md). If HPC, probe with `executor hpc-info` (it also reports GPU partitions/gres) and configure via `executor configure-execution`; offer GPU only when one is detected.
