...
def propose(run_dir: Path | str, plan: dict[str, Any]) -> dict[str, Any]:
    ...
    atac_pick = {"resolution": 0.8, "rationale": "default fallback", "stable_region": []}
    try:
        ...
        # Pull the LSI latent once for silhouette; subsample consistent with RNA path.
        atac_latent = None
        try:
            atac_latent = np.asarray(adata.obsm["X_lsi"]) if "X_lsi" in adata.obsm else None
        except Exception:
            atac_latent = None
        ...
        for res in atac_grid:
            ...
            # Silhouette on X_lsi (same semantics as RNA's silhouette on X_pca)
            sil = float("nan")
    ...
    if atac_rows:
        ...
        summary_lines.append("- (sweep not available)")
