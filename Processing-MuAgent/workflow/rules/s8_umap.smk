rule s8_umap_propose:
    input:
        ...
        rna_clustered = str(INTERNAL / "artifacts" / "s7_clustering" / "clustering_summary.json"),
    output:
...
rule s8_umap_execute:
    input:
        ...
        # Durable S7 marker, not the deletable rna_clustered.h5ad (read by path in-stage).
        rna_clustered    = str(INTERNAL / "artifacts" / "s7_clustering" / "clustering_summary.json"),
    output:
