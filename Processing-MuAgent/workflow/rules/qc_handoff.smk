# rna_only emits no peaks BED; ATAC-bearing branches declare it as a Snakemake output.
from executor import provenance as _provenance
_QC_HANDOFF_HAS_ATAC = _provenance.branch_has_atac(
    _provenance.current_branch(str(INTERNAL / "parameters.yaml"))
)

if _QC_HANDOFF_HAS_ATAC:
    rule qc_handoff:
        ...
        contract pointing at peaks BED (under deliverables/qc/) + prepared ATAC fragments). Gated on
        ...
        calls.parquet (the declared dep), joint_barcodes, overlap_summary, and prepared
        fragments are kept; the peaks BED is moved into deliverables/qc/.
        ...
        output:
            ...
            peaks    = str(QC / f"peaks_{RUN_DIR.name}.bed"),
        params:
            run_dir = str(RUN_DIR),
        ...
        run:
            from executor.stages.qc_handoff import run_snakemake_job
            run_snakemake_job(params.run_dir, input.plan)
else:
    rule qc_handoff:
        """Terminal post-QC handoff bundle for Integration-MuAgent (rna_only)."""
        input:
            approved  = str(INTERNAL / "checkpoints" / "post_qc_review.approved"),
            plan      = str(INTERNAL / "artifacts" / "p2_plan" / "preprocessing_plan.json"),
            calls     = str(INTERNAL / "artifacts" / "s3_doublets" / "calls.parquet"),
