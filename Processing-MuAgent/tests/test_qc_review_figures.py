...
class DeliverablesFigureLayoutTests(unittest.TestCase):
    def test_ensure_creates_plan_only_not_later_phases(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertFalse(paths.deliv_qc.exists())
    ...
    def test_migrate_moves_legacy_figures_into_central_dir(self):
        with tempfile.TemporaryDirectory() as tmp:
            paths = RunPaths(tmp)
            paths.ensure()
            # checkpoint/resolution_review is a still-supported legacy figure dir
            # (the obsolete checkpoint/qc_review figure dirs are no longer migrated
            # now that qc figures are written to deliverables/figures/ directly).
            legacy_dir = paths.deliverables / "checkpoint" / "resolution_review"
            legacy_dir.mkdir(parents=True, exist_ok=True)
            flat_png = legacy_dir / "post_qc_review_cell_counts.png"
